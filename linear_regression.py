...
def compute_Phi(x, p):
    ...
    Phi = np.matrix(np.empty([x.shape[0], p]))
...
def compute_L(yhat, y):
    ...
    for i in range(yhat.shape[0]):
        diff_sq = (yhat[i] - y[i])**2
        mean_sq = mean_sq + diff_sq

    L = mean_sq/(len(yhat) * 2)
...
def compute_dL_dw(y, yhat, Phi):
    ...
    #########################################
    # INSERT YOUR CODE HERE
    dL_dw = np.matrix(np.zeros([Phi.shape[1], 1]))

    for j in range(Phi.shape[1]):
        gradient = 0
        for i in range(Phi.shape[0]):
            update = (yhat[i] - y[i])*Phi.item(i, j) 
            gradient = gradient + update

        new_grad = gradient/(Phi.shape[0])
        dL_dw[j,:] = new_grad
